...
export function MedicalExtractionExample() {
  ...
  const handleTestCase = async () => {
    ...
    await startExtraction(
      "Paciente masculino presenta dolor de estómago", 
      sessionId
    )
    ...
    if (shouldContinue) {
      ...
      await continueExtraction(
        "El dolor mejora cuando el paciente duerme o descansa"
      )
  ...
  return (
    <div>
      <button onClick={handleTestCase}>
        🧪 Test Case: Hombre con dolor de estómago
      </button>
...
export async function directExtractionExample(dispatch: any) {
  ...
  try {
    ...
    const result = await dispatch(extractMedicalDataThunk({
      patientInput: "Paciente masculino de 15 años con dolor de estómago intensidad 7/10 desde la mañana, mejora con descanso",
      sessionId,
      isInitial: true
    })).unwrap()
