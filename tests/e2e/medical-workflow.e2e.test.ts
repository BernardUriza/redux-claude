...
test.describe('Medical Workflow E2E Tests', () => {
  ...
  test('Complete Medical Analysis Workflow', async ({ page }) => {
    ...
    test.step('Start medical consultation', async () => {
      ...
      await chatInput.fill(
        'Patient presents with severe headache, nausea, and blood pressure of 180/110 mmHg. Symptoms started 3 days ago.'
      )
    ...
    test.step('Verify SOAP Analysis appears', async () => {
      ...
      const soapDisplay = page.locator(
        '[data-testid="soap-display"], .soap-display, :has-text("SOAP"), :has-text("Subjetivo")'
      )
      ...
      await expect(
        page.locator(':has-text("headache"), :has-text("nausea"), :has-text("blood pressure")')
      ).toBeVisible()
    })
    ...
    test.step('Verify Real-Time Metrics', async () => {
      ...
      const metricsComponent = page.locator(
        '[data-testid="real-time-metrics"], :has-text("Métricas"), :has-text("Confianza")'
      )
    ...
    test.step('Verify Diagnostic Progress', async () => {
      ...
      const progressComponent = page.locator(
        '[data-testid="diagnostic-progress"], :has-text("Progreso"), :has-text("Diagnóstico")'
      )

      // Verificar si el componente aparece (puede no aparecer si no hay suficiente progreso)
      const isVisible = await progressComponent
        .first()
    ...
    test.step('Verify Follow-up Reminders', async () => {
      ...
      const followUpComponent = page.locator(
        '[data-testid="follow-up"], :has-text("Seguimiento"), :has-text("Recordatorio")'
      )
    ...
    test.step('Verify Medical Notes Generation', async () => {
      ...
      const notesComponent = page.locator(
        '[data-testid="medical-notes"], :has-text("Notas Médicas"), :has-text("Medical Notes")'
      )
      await expect(notesComponent.first()).toBeVisible({ timeout: 10000 })

      // Verificar que se generan notas a partir de los mensajes
      const notesExist = await page
        .locator('.note-card, [class*="note"], :has-text("Sistema Multinúcleo")')
        .count()
      if (notesExist > 0) {
    ...
    test.step('Monitor system alerts', async () => {
      ...
      if (alertCount > 0) {
        ...
        const closeButton = firstAlert.locator(
          'button:has-text("✕"), button:has-text("Ocultar"), button:has-text("Hide")'
        )
  ...
  test('System Health Dashboard', async ({ page }) => {
    test.step('Verify dashboard displays system health', async () => {
      ...
      const dashboard = page.locator(
        '[data-testid="health-dashboard"], :has-text("Sistema Médico"), :has-text("Health")'
      )
      ...
      if (await dashboard.first().isVisible({ timeout: 5000 })) {
        ...
        await expect(
          page.locator(':has-text("CPU"), :has-text("Memoria"), :has-text("Memory")')
        ).toBeVisible()
        ...
        if (await expandButton.isVisible()) {
          ...
          await expect(
            page.locator('button:has-text("Contraer"), button:has-text("Contract")')
          ).toBeVisible()
        }
      } else {
  ...
  test('Interactive Medical Conversation Flow', async ({ page }) => {
    ...
    test.step('Verify components update with new data', async () => {
      ...
      await expect(
        page.locator(':has-text("chest pain"), :has-text("shortness"), :has-text("ST elevation")')
      ).toBeVisible()
      ...
      const progressExists = await page
        .locator(':has-text("Progreso"), :has-text("Ciclo")')
        .isVisible({ timeout: 5000 })
      if (progressExists) {
  ...
  test('Performance and Lazy Loading', async ({ page }) => {
    test.step('Verify lazy loading components', async () => {
      ...
      const loadingElements = page.locator(
        '[class*="skeleton"], [class*="loading"], :has-text("Cargando")'
      )
    ...
    test.step('Test error states', async () => {
      ...
      if (await chatInput.isVisible({ timeout: 5000 })) {
        ...
        const errorElements = page.locator(
          ':has-text("error"), :has-text("Error"), :has-text("problema")'
        )
