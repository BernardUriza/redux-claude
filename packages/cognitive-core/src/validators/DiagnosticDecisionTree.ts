...
export class DiagnosticDecisionTree {
  ...
  getRequiredStudies(
    ...
    this.CONDITIONAL_STUDIES.forEach(rule => {
      ...
      const contextTriggered = rule.trigger.some(
        trigger => patientContext.includes(trigger) || allFindings.some(f => f.includes(trigger))
      )
  ...
  validateStudyCompleteness(
    ...
    requiredStudies.mandatory.forEach(rule => {
      const isIncluded = proposedLower.some(
        proposed =>
