...
export class DefensiveMedicineValidator {
  ...
  identifyUrgentPatterns(input: string): UrgentPattern[] {
    ...
    this.urgentPatterns.forEach((pattern, key) => {
      ...
      const hasRedFlags = pattern.redFlags.some(flag => inputLower.includes(flag.toLowerCase()))
  ...
  private createDefensiveDiagnosis(differential: any): DefensiveDiagnosis {
    ...
    const actionRequired = this.generateActionPlan(
      gravityScore,
      differential.diagnosis || differential.name
    )
  ...
  private assessGravityScore(diagnosis: string): number {
    ...
    if (
      diagnosisLower.includes('gastritis') ||
      diagnosisLower.includes('migraña') ||
      diagnosisLower.includes('infección urinaria') ||
      diagnosisLower.includes('bronquitis')
    ) {
      return 5
    }
  ...
  generateDefensivePrompt(urgentPatterns: UrgentPattern[], overallUrgency: any): string {
    const patternsText = urgentPatterns
      .map(p => `- ${p.criticalDifferentials.join(', ')}`)
      .join('\n')
