...
export class UrgencyClassifier {
  ...
  private riskModifiers = {
    age: {
      pediatric: { modifier: 1.2, patterns: ['fiebre', 'deshidratación', 'dificultad respiratoria'] },
  ...
  async assessUrgency(
    ...
    const escalationCriteria = this.generateEscalationCriteria(adjustedUrgency.level, identifiedPatterns)
    
    // Paso 5: Crear prompt defensivo
    const defensivePrompt = this.defensiveValidator.generateDefensivePrompt(identifiedPatterns, adjustedUrgency)
    ...
    if (patientContext?.age) {
      if (patientContext.age < 18) {
        riskFactors.push({
          ...
          riskLevel: 'high',
          impact: 'Presentación atípica de patologías, compensación rápida seguida de descompensación súbita',
   ...
   */
  private generateEscalationCriteria(
    urgencyLevel: string, 
    patterns: UrgentPattern[]
  ): string[] {
  ...
  private requiresSpecialist(assessment: UrgencyAssessment): boolean {
    ...
    const specialistPatterns = [
      'infarto', 'embolia', 'hemorragia', 'disección',
      'meningitis', 'sepsis', 'ictus', 'convulsiones'
    ]
  ...
  private mapUrgencyToTime(level: string): string {
    switch (level) {
      case 'critical': return 'Inmediato (< 15 min)'
      case 'high': return 'Urgente (< 1 hora)'
      case 'medium': return 'Prioritario (< 4 horas)'
      default: return 'Rutinario (< 24 horas)'
    }
  }
