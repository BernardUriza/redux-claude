...
const decisionsSlice = createSlice({
  ...
  reducers: {
    ...
    addAuditEntries: (state, action: PayloadAction<AuditEntry[]>) => {
      ...
      state.sessionQuality.clinicalCoherence = Math.max(
        0,
        100 - (critical * 50) / total - (warnings * 20) / total
      )
      state.sessionQuality.safetyScore = Math.max(0, 100 - critical * 30 - warnings * 10)
      state.sessionQuality.decisionQuality = Math.max(
        0,
        100 - (critical * 40) / total - (warnings * 15) / total
      )
