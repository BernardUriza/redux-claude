...
const isAssistantMedicalMessage = (action: AnyAction): boolean => {
  return (
    action.type === 'medicalChatEvolved/addAssistantMessage' ||
    (action.type === 'medicalChatEvolved/addDashboardMessage' &&
      action.payload.type === 'assistant')
  )
}
...
const hasSignificantMedicalContent = (content: string): boolean => {
  ...
  return medicalKeywords.some(keyword => contentLower.includes(keyword)) && content.length > 30
}
...
const extractSOAPFromMessages = async (
  ...
  const medicalMessages = messages.filter(
    msg => msg.id !== 'welcome_multinucleus' && hasSignificantMedicalContent(msg.content)
  )
  ...
  const subjective = userMessages.length > 0 ? userMessages.map(m => m.content).join('\n\n') : null
  ...
  const assessment =
    assistantMessages
      .filter(
        m =>
          m.content.toLowerCase().includes('diagnos') || m.content.toLowerCase().includes('assess')
      )
      .map(m => m.content)
      .join('\n\n') || null
  ...
  const plan =
    assistantMessages
      .filter(
        m => m.metadata?.sectionType === 'treatment' || m.metadata?.sectionType === 'followup'
      )
      .map(m => m.content)
      .join('\n\n') || null

  // Calcular confianza y calidad
  const confidenceScores = medicalMessages.filter(m => m.confidence).map(m => m.confidence)
...
export const createMedicalSyncMiddleware = (config: Partial<MiddlewareConfig> = {}) => {
  ...
  return ((store: any) => (next: any) => async (action: any) => {
    ...
    if (isAssistantMedicalMessage(action) && fullConfig.autoSOAPExtraction && !pendingExtraction) {
      const state = store.getState()
      ...
      const medicalCount = dashboardMessages.filter(
        (msg: any) => msg.id !== 'welcome_multinucleus' && hasSignificantMedicalContent(msg.content)
      ).length
      ...
      if (medicalCount >= fullConfig.extractionThreshold) {
        ...
        try {
          ...
          const { analysis, quality } = await extractSOAPFromMessages(dashboardMessages, sessionId)
...
export const createTestingMiddleware = () => createMedicalSyncMiddleware(medicalSyncConfigs.testing)
