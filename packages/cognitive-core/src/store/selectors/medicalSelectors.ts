...
const extractSubjectiveFromMessages = (messages: MedicalMessage[]): string | null => {
  ...
  const subjectiveContent = userMessages
    .map(msg => msg.content)
    .join('\n\n')
...
const extractObjectiveFromMessages = (messages: MedicalMessage[]): string | null => {
  const assistantMessages = messages.filter(msg => 
    ...
    msg.content.includes('observ') || msg.content.includes('exam') || msg.content.includes('sign')
  )
  
  if (assistantMessages.length === 0) return null
  
  const objectiveContent = assistantMessages
    .map(msg => msg.content)
    .join('\n\n')
...
const extractAssessmentFromMessages = (messages: MedicalMessage[]): string | null => {
  const diagnosticMessages = messages.filter(msg => 
    msg.type === 'assistant' && 
    (msg.metadata?.sectionType === 'diagnosis' || 
  ...
  const assessmentContent = diagnosticMessages
    .map(msg => msg.content)
    .join('\n\n')
...
const extractPlanFromMessages = (messages: MedicalMessage[]): string | null => {
  ...
  const planContent = planMessages
    .map(msg => msg.content)
    .join('\n\n')
...
const determineSystemHealth = (confidence: number, activeCores: number, responseTime: number): SystemMetrics['systemHealth'] => {
...
const extractCompletedPhases = (messages: MedicalMessage[]): DiagnosticPhase[] => {
  ...
  if (messages.some(m => m.type === 'assistant' && m.metadata?.sectionType === 'diagnosis')) phases.push('analysis')
  if (messages.some(m => m.metadata?.sectionType === 'diagnosis' && m.confidence && m.confidence > 0.7)) phases.push('diagnosis')
...
const determineCurrentPhase = (messages: MedicalMessage[], completed: DiagnosticPhase[]): DiagnosticPhase => {
...
export const selectSystemMetrics = createSelector(
  ...
  (cores, sharedState): SystemMetrics => {
    ...
    const healthScore = systemHealth === 'optimal' ? 95 : 
                       systemHealth === 'good' ? 80 :
                       systemHealth === 'warning' ? 60 : 30
...
export const selectDiagnosticProgress = createSelector(
  ...
  (messages, lastActivity): DiagnosticProgress => {
    ...
    return {
      ...
      currentPhase,
      phasesCompleted,
      nextPhase: currentPhase === 'followup' ? null : 
                 currentPhase === 'treatment' ? 'followup' :
                 currentPhase === 'diagnosis' ? 'treatment' :
                 currentPhase === 'analysis' ? 'diagnosis' : 'analysis',
      completionPercentage,
...
export const selectPatientReminders = createSelector(
  ...
  (dashboardMessages, assistantMessages): PatientReminder[] => {
    ...
    followupMessages.forEach((message, index) => {
      ...
      // Clasificar tipo
      if (content.includes('medicament') || content.includes('pill') || content.includes('dose')) type = 'medication'
      if (content.includes('appointment') || content.includes('cita') || content.includes('visit')) type = 'appointment'  
      if (content.includes('test') || content.includes('analysis') || content.includes('exam')) type = 'test'
      if (content.includes('exercise') || content.includes('diet') || content.includes('lifestyle')) type = 'lifestyle'
      if (content.includes('monitor') || content.includes('check') || content.includes('watch')) type = 'monitoring'
      
      // Determinar prioridad
      if (content.includes('urgent') || content.includes('important') || content.includes('critical')) priority = 'urgent'
      ...
      reminders.push({
        ...
        estimatedDuration: type === 'medication' ? 5 : 
                          type === 'appointment' ? 60 : 
                          type === 'test' ? 120 : 30,
        medicalContext: message.content,
        extractedFromPhase: message.metadata?.sectionType === 'treatment' ? 'treatment' : 'followup'
      })
    })
  ...
  (messages, session): PhysicianNote[] => {
    ...
    const notes: PhysicianNote[] = noteWorthyMessages.map((message, index) => {
      ...
      const category: NoteCategory = 
        message.metadata?.sectionType === 'diagnosis' ? 'diagnosis' :
        message.metadata?.sectionType === 'treatment' ? 'treatment' :
        message.metadata?.sectionType === 'followup' ? 'followup' :
        message.metadata?.sectionType === 'education' ? 'education' : 'observation'
      ...
      const isImportant = (message.confidence || 0) > 0.8 || 
