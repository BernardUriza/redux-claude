...
export class MedicalAuditEngine {
    ...
    if (redFlags.length > 0 && priority === 'low') {
      return {
        ...
        details: {
          ...
          reasoning:
            'Red flags indicate potential serious conditions that require urgent attention',
          recommendations: [
            'Escalate priority to HIGH or CRITICAL',
            'Review red flag assessment',
    ...
    if (!redFlags && priority === 'high') {
      return {
        ...
        details: {
          ...
          recommendations: [
            'Review for potential red flags',
            'Document why high priority assigned',
          ],
      ...
      if (!hasOverlap && previous.length > 0) {
        return {
          ...
          details: {
            ...
            recommendations: [
              'Review clinical progression',
              'Document reasoning for diagnostic shift',
              'Consider if new information justifies change',
            ],
      ...
      if (testsNeeded.length === 0 && decision.confidence < 80) {
        return {
          ...
          details: {
            ...
            recommendations: [
              'Consider diagnostic tests to increase confidence',
      ...
      if (!expectedTimeframes.includes(followUpTimeframe)) {
        return {
          ...
          details: {
            ...
            recommendations: [
              `Adjust priority to match ${followUpTimeframe} timeframe`,
              `Or adjust timeframe to match ${priority} priority`,
            ],
  ...
  private updateOverallAssessment(): void {
    ...
    this.sessionAudit.overallAssessment.clinicalCoherence = Math.max(
      0,
      100 - criticalPenalty - warningPenalty
    )
    this.sessionAudit.overallAssessment.safetyScore = Math.max(
      0,
      100 - criticalCount * 30 - warningCount * 10
    )
    this.sessionAudit.overallAssessment.decisionQuality = Math.max(
      0,
      100 - criticalPenalty - warningPenalty
    )
