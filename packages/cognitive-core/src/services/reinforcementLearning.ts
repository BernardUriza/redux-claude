...
export class ReinforcementLearningSystem {
  ...
  private updatePerformanceMetrics(result: DecisionResult, context: string): void {
    ...
    if (result.success) {
      metrics.avgConfidence = alpha * (result.confidence / 100) + 
                              (1 - alpha) * metrics.avgConfidence
    }
  ...
  private detectTrend(agentType: AgentType): void {
    const recentRewards = this.learning.rewards
      .filter(r => r.agentType === agentType)
      .slice(0, 10)
    
    if (recentRewards.length < 5) return
    
    const avgRecent = recentRewards.slice(0, 5)
      .reduce((sum, r) => sum + r.reward, 0) / 5
    const avgPrevious = recentRewards.slice(5, 10)
      .reduce((sum, r) => sum + r.reward, 0) / Math.min(5, recentRewards.length - 5)
  ...
  private adjustPriority(agentType: AgentType, context: string): LearningUpdate | null {
    ...
    if (Math.random() < this.learning.explorationRate) {
      ...
      const adjustment = this.learning.learningRate * 
                         (performanceScore - 0.5) * 
                         trendFactor * 
                         10 // Escalar a rango de prioridad
  ...
  getSystemLearningStats(): {
    ...
    const overallSuccess = allMetrics.reduce(
      (sum, [, metrics]) => sum + metrics.successRate, 0
    ) / allMetrics.length
    
    const avgLatency = allMetrics.reduce(
      (sum, [, metrics]) => sum + metrics.avgLatency, 0
    ) / allMetrics.length
    ...
    const totalRewards = this.learning.rewards.reduce(
      (sum, r) => sum + Math.max(0, r.reward), 0
    )
  ...
  calibrateSystem(): void {
    ...
    const stableAgents = Object.values(this.learning.agentPerformance)
      .filter(m => m.trend === 'stable').length
