...
const createInitialCircuitBreaker = (config?: Partial<CircuitBreakerConfig>): AgentCircuitBreaker => ({
...
const calculateGlobalMetrics = (state: AgentCircuitBreakersState) => {
  ...
  const failedAgents = Object.values(state.agentStatuses).filter(status => status === AgentStatus.FAILED).length
  const activeAgents = Object.values(state.agentStatuses).filter(status => 
...
const agentCircuitBreakersSlice = createSlice({
  ...
  reducers: {
    // Agent call started
    agentCallStarted: (state, action: PayloadAction<{ agentType: AgentType, requestId: string }>) => {
    ...
    agentCallSucceeded: (state, action: PayloadAction<{ 
      agentType: AgentType
      requestId: string
      latency: number
      confidence: number
    ...
    // Agent call failed
    agentCallFailed: (state, action: PayloadAction<{ 
      agentType: AgentType
      requestId: string
      error: string
      latency?: number
      ...
      // Update status
      state.agentStatuses[agentType] = breaker.state === CircuitBreakerState.OPEN 
        ? AgentStatus.FAILED 
        : AgentStatus.READY
    ...
    updateAgentConfig: (state, action: PayloadAction<{ 
      agentType: AgentType
    ...
    performHealthCheck: (state) => {
      ...
      Object.keys(state.circuitBreakers).forEach(agentTypeKey => {
        ...
        if (breaker.lastFailure && 
            now - breaker.lastFailure > breaker.config.resetTimeoutMs) {
