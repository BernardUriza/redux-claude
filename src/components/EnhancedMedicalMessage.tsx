...
const SOAPBadge = ({ section }: { section: string }) => {
  ...
  return (
    <div className={`inline-flex items-center space-x-2 bg-gradient-to-r ${config.color} bg-opacity-20 px-3 py-1 rounded-full border border-opacity-30 mb-2`}>
...
const ConfidenceIndicator = ({ confidence }: { confidence: number }) => {
  ...
  return (
    <div className={`inline-flex items-center space-x-2 bg-gradient-to-r ${getConfidenceColor(confidence)} bg-opacity-20 px-3 py-1 rounded-full border border-opacity-30`}>
...
const AgentInsights = ({ content }: { content: string }) => {
  ...
  const cognitiveMatch = content.match(/## 🧠 ANÁLISIS MULTI-AGENTE COGNITIVO([\s\S]*?)(?=\n##|\n\*|$)/)
  
  if (!cognitiveMatch) return null

  const cognitiveContent = cognitiveMatch[1]
  const agentsConsulted = cognitiveContent.match(/\*\*🎯 Agentes Especializados Consultados:\*\* (\d+)/)?.[1]
  const consensus = cognitiveContent.match(/\*\*🤝 Consenso Alcanzado:\*\* (✅ SÍ|❌ NO)/)?.[1]
  const validation = cognitiveContent.match(/\*\*🧩 Validación Especializada:\*\* (✅ ACTIVADA|❌ NO APLICADA)/)?.[1]
...
export const EnhancedMedicalMessage = ({ message, isStreaming = false }: MedicalMessageProps) => {
  ...
  const isSOAPAnalysis = message.content.includes('EXPEDIENTE CLÍNICO') || message.content.includes('### S - SUBJETIVO')
  
  // Detect if it's a rejection message
  const isRejection = message.content.includes('⚠️ Consulta No Válida')

  return (
    <div className={`flex gap-4 p-6 ${isAssistant ? 'bg-gray-800/30' : 'bg-gray-900'} border-b border-gray-800 transition-all duration-300`}>
      {/* Enhanced Avatar */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="prose prose-invert prose-sm max-w-none">
          <ReactMarkdown
            components={{
              h1: ({children}) => <h1 className="text-2xl font-bold mb-4 text-white border-b-2 border-blue-500 pb-3">{children}</h1>,
              h2: ({children}) => {
                const text = String(children)
                if (text.includes('EXPEDIENTE CLÍNICO')) {
                  return <h2 className="text-xl font-bold mb-4 text-blue-300 bg-blue-900/20 p-3 rounded-lg border border-blue-500/30">{children}</h2>
                }
                if (text.includes('S - SUBJETIVO')) {
                  return <h2 className="text-lg font-bold mb-3 text-blue-300 flex items-center gap-2"><SOAPBadge section="S" />{children}</h2>
                }
                if (text.includes('O - OBJETIVO')) {
                  return <h2 className="text-lg font-bold mb-3 text-emerald-300 flex items-center gap-2"><SOAPBadge section="O" />{children}</h2>
                }
                if (text.includes('A - ANÁLISIS')) {
                  return <h2 className="text-lg font-bold mb-3 text-purple-300 flex items-center gap-2"><SOAPBadge section="A" />{children}</h2>
                }
                if (text.includes('P - PLAN')) {
                  return <h2 className="text-lg font-bold mb-3 text-orange-300 flex items-center gap-2"><SOAPBadge section="P" />{children}</h2>
                }
                return <h2 className="text-lg font-bold mb-3 text-white">{children}</h2>
              },
              h3: ({children}) => <h3 className="text-base font-semibold mb-2 text-cyan-200">{children}</h3>,
              ...
              li: ({children}) => <li className="text-gray-300 list-disc marker:text-blue-400">{children}</li>,
              strong: ({children}) => <strong className="font-semibold text-white bg-slate-800/50 px-1 rounded">{children}</strong>,
              em: ({children}) => <em className="italic text-blue-300">{children}</em>,
              code: ({children}) => <code className="bg-slate-800 px-2 py-1 rounded text-sm font-mono text-cyan-300 border border-slate-600">{children}</code>,
              hr: () => <hr className="my-6 border-gray-700" />,
              blockquote: ({children}) => <blockquote className="border-l-4 border-blue-500 pl-6 my-4 text-gray-400 italic bg-blue-900/10 py-2 rounded-r">{children}</blockquote>
            }}
          >
            {message.content}
          </ReactMarkdown>
