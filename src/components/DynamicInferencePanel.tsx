...
export const DynamicInferencePanel: React.FC<DynamicInferencePanelProps> = ({ 
  ...
  const getUrgencyColor = (urgency: string): string => {
    switch (urgency.toLowerCase()) {
      case 'crítico': return 'text-red-500 bg-red-900/30 border-red-700/50'
      case 'alto': return 'text-orange-500 bg-orange-900/30 border-orange-700/50'
      case 'medio': return 'text-yellow-500 bg-yellow-900/30 border-yellow-700/50'
    ...
    <div className={`bg-gray-900 border border-gray-700 rounded-lg shadow-xl ${className}`}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-indigo-200 text-xs">Datos del paciente se actualizan automáticamente</p>
            </div>
          </div>
      ...
      <div className="p-5 space-y-4">
        {inferences.map((inference) => (
          <div key={inference.id} className="bg-gray-800 rounded-lg p-4 border border-gray-600 hover:border-gray-500 transition-colors">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${inference.confidence > 0.7 ? 'bg-green-400' : inference.confidence > 0.4 ? 'bg-yellow-400' : 'bg-gray-400'}`}></div>
                <span className="font-medium text-gray-200">{inference.label}</span>
              </div>
              <span className={`text-xs px-3 py-1 rounded-full border font-semibold ${getConfidenceColor(inference.confidence)}`}>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                {inference.isEditable && inference.type === 'age' ? (
                  <input
                    ...
                    onChange={(e) => handleInferenceChange(inference.id, parseInt(e.target.value) || 0)}
                  ...
                  <div className={`flex-1 px-4 py-2 rounded-lg border font-medium ${
              ...
              <div className="mt-2">
                <div className="flex justify-between text-xs text-gray-400 mb-1">
                  <span>Confianza</span>
                  <span>{Math.round(inference.confidence * 100)}% 
                ...
                <div className="w-full bg-gray-600 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full transition-all duration-700 ${
                      inference.confidence > 0.8 ? 'bg-gradient-to-r from-green-500 to-emerald-400' :
                      inference.confidence > 0.6 ? 'bg-gradient-to-r from-yellow-500 to-amber-400' :
