...
const FollowUpTracker = lazy(() =>
  import('./FollowUpTracker').then(m => ({ default: m.FollowUpTracker }))
)
const MedicalNotes = lazy(() => import('./MedicalNotes').then(m => ({ default: m.MedicalNotes })))

// 📊 LOADING SKELETON COMPONENTE
const MedicalComponentSkeleton = ({
  height = '200px',
  title,
}: {
  height?: string
  ...
  <div
    className="bg-slate-800/30 backdrop-blur-sm rounded-xl p-6 border border-slate-600/30 animate-pulse"
    style={{ height }}
  >
...
export const LazyFollowUpTracker = () => (
  <Suspense
    fallback={<MedicalComponentSkeleton height="500px" title="Seguimiento y Recordatorios" />}
  >
    <FollowUpTracker />
  </Suspense>
)
